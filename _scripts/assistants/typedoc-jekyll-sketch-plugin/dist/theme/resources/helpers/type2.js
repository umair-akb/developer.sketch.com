...
function type(collapse = 'none', emphasis = true) {
    ...
    return this ? '`' + escape_1.code_escape(this.toString().split(' | ').join('` | `')) + '`' : '';
}
exports.type = type;
    ...
    const functions = modelSignatures.map((fn) => {
        ...
        const params = fn.parameters
            ? fn.parameters.map((param) => {
                return `${param.flags.isRest ? '...' : ''}\`${param.name}${param.flags.isOptional ? '?' : ''}\`: ${type.call(param.type ? param.type : param)}`;
            })
            : [];
...
function getReferenceType(model, emphasis) {
    if (model.reflection || (model.name && model.typeArguments)) {
        const reflection = model.reflection && model.reflection.url
            ? [
                `[*${escape_1.code_escape(model.reflection.name)}*](${theme_1.default.HANDLEBARS.helpers.relativeURL(model.reflection.url)})`,
            ]
            : [escape_1.code_escape(model.name)];
    ...
    return escape_1.code_escape(model.name);
}
...
function getUnionType(model, emphasis) {
    return '`' + model.types
        .map((unionType) => type.call(unionType, 'none', false))
        .join(`\` | \``) + '`';
}
...
function getIntrinsicType(model, emphasis) {
    return emphasis ? `*${escape_1.code_escape(model.name)}*` : '`' + escape_1.code_escape(model.name) + '`';
}
...
function getInferredType(model) {
    return `*infer* ${escape_1.code_escape(model.name)}`;
}
